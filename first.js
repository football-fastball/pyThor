(function(){function range(start,stop,step){if(arguments.length<=1){stop=start||0;start=0}step=arguments[2]||1;var length=Math.max(Math.ceil((stop-start)/step),0);var idx=0;var range=new Array(length);while(idx<length){range[idx++]=start;start+=step}return range}function len(obj){if(obj instanceof Array||typeof obj==="string")return obj.length;else{var count=0;for(var i in obj){if(obj.hasOwnProperty(i))count++}return count}}function _$rapyd$_in(val,arr){if(arr instanceof Array||typeof arr==="string")return arr.indexOf(val)!=-1;else{if(arr.hasOwnProperty(val))return true;return false}}function dir(item){var arr=[];for(var i in item){arr.push(i)}return arr}function _$rapyd$_extends(child,parent){child.prototype=new parent;child.prototype.constructor=child}function _$rapyd$_print(){var args,output;args=[].slice.call(arguments,0);output=JSON.stringify(args);if("console"in window)console.log(output.substr(1,output.length-2))}var JSON,str,a,b,i;JSON=JSON||{};if(!JSON.stringify){
	...
	Array.prototype.filter = function(filterfun, thisArg) {
        ...
        ;return keys}}else{dict.keys=function(hash){return Object.getOwnPropertyNames(hash)}}dict.values=function(hash){var vals,key;vals=[];var _$rapyd$_Iter3=dict.keys(hash);for(var _$rapyd$_Index3=0;_$rapyd$_Index3<_$rapyd$_Iter3.length;_$rapyd$_Index3++){key=_$rapyd$_Iter3[_$rapyd$_Index3];vals.append(hash[key])}return vals};dict.items=function(hash){var items,key;items=[];var _$rapyd$_Iter4=dict.keys(hash);for(var _$rapyd$_Index4=0;_$rapyd$_Index4<_$rapyd$_Iter4.length;_$rapyd$_Index4++){key=_$rapyd$_Iter4[_$rapyd$_Index4];items.append([key,hash[key]])}return items};dict.copy=dict;dict.clear=function(hash){var key;var _$rapyd$_Iter5=dict.keys(hash);for(var _$rapyd$_Index5=0;_$rapyd$_Index5<_$rapyd$_Iter5.length;_$rapyd$_Index5++){key=_$rapyd$_Iter5[_$rapyd$_Index5];delete hash[key]}};function add(where,what,id){if(typeof id==="undefined")id=false;if(id){$(where).prepend($(what).attr("id",id))}else{$(where).prepend($(what))}}function add_append(where,what,id){if(typeof id==="undefined")id=false;if(id){$(where).append($(what).attr("id",id))}else{$(where).append($(what))}}function css(id,prop,value){$("#"+id).css(prop,value)}function css_by_tag(el,prop,value){$(el).css(prop,value)}_$rapyd$_print(" hello world, and then some ");a="I was here";b="and there";for(i=0;i<10;i++){_$rapyd$_print(a,b)}css_by_tag("body","margin","0px");add("body","<canvas height=\"20\" width=\"150\"></canvas>","thrive");css("thrive","border","1px dotted black")})();